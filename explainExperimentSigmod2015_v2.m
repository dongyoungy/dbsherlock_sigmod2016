...
% % 	lagged_abnormal_matrix = {};
	...
	%if exp_param.introduce_lag
		%for j=3:numAttr
			%train_data = [];
			%for i=1:numRow
				%if ismember(i, lagged_abnormal_indexes{j})
					%train_data(rowCount,:) = [data(i,:) -1]; % -1 if abnormal
					%rowCount = rowCount + 1;
				%end
				%if ismember(i, lagged_normal_indexes{j})
					%train_data(rowCount,:) = [data(i,:) 1]; % 1 if normal
					%rowCount = rowCount + 1;
				%end
			%end
			%lagged_training_data{j} = train_data;
		%end
	%end
	...
	%size_of_causal_model = size(causalModels,2)
	...
	for i=1:size(causalModels,2)
		...
		%size_of_predicates = size(effectPredicates, 1)
